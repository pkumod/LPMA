...
void rebalance_batch(SIZE_TYPE level, SIZE_TYPE seg_length, KEY_PTR keys[], VALUE_PTR values[],
        ...
        SIZE_TYPE lower_bound, SIZE_TYPE upper_bound, SIZE_TYPE tree_height, SIZE_TYPE *csr_idx, SIZE_TYPE *csr_lane, SIZE_TYPE thres) {
...
void show_rpma(RPMA &rpma) {
    for (int i = 0; i <= rpma.tree_height; ++i) {
		...
		if (j % rpma.segment_length == 0)
            ...
            std::cout << "\t" << j << "\t" << h_keys[j] << "\t" << h_values[j] << std::endl;
        }
	std::cout << std::endl;
        delete []h_keys;
        delete []h_values;
...
void update_rpma(RPMA &rpma, DEV_VEC_KEY &update_keys, DEV_VEC_VALUE &update_values, SIZE_TYPE thres) {
...
//std::cout << "trying to insert in 1height" << level << std::endl;
        // re-balance
        rebalance_batch(level, rpma.segment_length, RAW_PTR(rpma.levels_key_ptr_array), RAW_PTR(rpma.levels_value_ptr_array), RAW_PTR(update_nodes),
                RAW_PTR(update_keys), RAW_PTR(update_values), update_size, RAW_PTR(unique_update_nodes),
                RAW_PTR(update_offset), unique_node_size, lower_bound, upper_bound, rpma.tree_height, RAW_PTR(rpma.csr_idx), RAW_PTR(rpma.csr_lane), thres);
...
//assert(update_keys.size() == update_size);
    ...
    if (update_size > 0) {
        ...
        SIZE_TYPE original_tree_height = rpma.tree_height;
        SIZE_TYPE updated_tree_height = resize_rpma(rpma, update_keys, update_values, update_size, thres);
    	if (updated_tree_height < original_tree_height || updated_tree_height - original_tree_height > 1) {
            ...
            SIZE_TYPE lower_bound = rpma.lower_element[rpma.tree_height+1];
            SIZE_TYPE upper_bound = rpma.upper_element[rpma.tree_height+1];

            rebalance_batch(rpma.tree_height + 1, rpma.segment_length, RAW_PTR(rpma.levels_key_ptr_array), RAW_PTR(rpma.levels_value_ptr_array), RAW_PTR(update_nodes),
                    RAW_PTR(update_keys), RAW_PTR(update_values), update_size, RAW_PTR(unique_update_nodes),
                    RAW_PTR(update_offset), unique_node_size, lower_bound, upper_bound, rpma.tree_height, RAW_PTR(rpma.csr_idx), RAW_PTR(rpma.csr_lane), thres);
            cErr(cudaDeviceSynchronize());
...
void get_data_rpma(RPMA &rpma, KEY_PTR &h_keys, VALUE_PTR &h_values) {
    	SIZE_TYPE size = rpma.segment_length * ((2 << rpma.tree_height) - 1);
...
void init_csr_rpma(RPMA &rpma, SIZE_TYPE row_num) {
    ...
    init_rpma(rpma);
    cErr(cudaDeviceSynchronize());
    update_rpma(rpma, row_wall, tmp_value, 0);  
}
