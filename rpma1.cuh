...
void update_rpma(RPMA &rpma, DEV_VEC_KEY &update_keys, DEV_VEC_VALUE &update_values, SIZE_TYPE thres, DEV_VEC_SIZE &stat) {
...
//std::cout << "a" << (32 << level) << std::endl;
        rebalance_batch(level, rpma.segment_length, RAW_PTR(rpma.levels_key_ptr_array), RAW_PTR(rpma.levels_value_ptr_array), RAW_PTR(update_nodes),
                ...
                RAW_PTR(update_offset), unique_node_size, lower_bound, upper_bound, rpma.tree_height, RAW_PTR(rpma.csr_idx), RAW_PTR(rpma.csr_lane), thres, stat);
...
__host__
void init_csr_rpma(RPMA &rpma, SIZE_TYPE row_num, DEV_VEC_SIZE &stat) {
    ...
    init_rpma(rpma);
    cErr(cudaDeviceSynchronize());
    update_rpma(rpma, row_wall, tmp_value, 0, stat);  
}
