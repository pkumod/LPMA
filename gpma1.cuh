...
/*
        ...
        level_length <<= 1;
*/
	gpma.lower_element[i] = gpma.density_lower_thres_root * level_length;
	gpma.upper_element[i] = level_length;
	level_length <<= 1;
    }
}
...
void block_rebalancing_kernel(SIZE_TYPE seg_length, SIZE_TYPE level, KEY_TYPE *keys, VALUE_TYPE *values,
        ...
        SIZE_TYPE *update_offset, SIZE_TYPE lower_bound, SIZE_TYPE upper_bound, SIZE_TYPE *row_offset, SIZE_TYPE *stat) {
...
void rebalancing_kernel(SIZE_TYPE unique_update_size, SIZE_TYPE seg_length, SIZE_TYPE level, KEY_TYPE *keys,
        ...
        SIZE_TYPE *row_offset, SIZE_TYPE *stat) {
...
void rebalance_batch(SIZE_TYPE level, SIZE_TYPE seg_length, KEY_TYPE *keys, VALUE_TYPE *values,
        ...
        SIZE_TYPE lower_bound, SIZE_TYPE upper_bound, SIZE_TYPE *row_offset, DEV_VEC_SIZE &stat) {
    ...
    if (update_width <= 1024) {
        ...
        func_arr[fls(update_width) - 2]<<<BLOCKS_NUM, THREADS_NUM>>>(seg_length, level, keys, values, update_nodes,
                update_keys, update_values, unique_update_nodes, update_offset, lower_bound, upper_bound, row_offset, RAW_PTR(stat));
    } else {
...
__host__
void update_gpma(GPMA &gpma, DEV_VEC_KEY &update_keys, DEV_VEC_VALUE &update_values, DEV_VEC_SIZE &stat) {
...
//std::cout << (32 << level) << std::endl;
        ...
        rebalance_batch(level, gpma.segment_length, RAW_PTR(gpma.keys), RAW_PTR(gpma.values), RAW_PTR(update_nodes),
                ...
                RAW_PTR(update_offset), unique_node_size, lower_bound, upper_bound, RAW_PTR(gpma.row_offset), stat);
...
__host__
void init_csr_gpma(GPMA &gpma, SIZE_TYPE row_num, DEV_VEC_SIZE &stat) {
    ...
    init_gpma(gpma);
    cErr(cudaDeviceSynchronize());
    update_gpma(gpma, row_wall, tmp_value, stat);
}
